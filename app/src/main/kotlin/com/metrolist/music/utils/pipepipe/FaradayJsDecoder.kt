package com.metrolist.music.utils.pipepipe

import com.metrolist.music.utils.pipepipe.FaradayCipherEngine
import com.metrolist.music.utils.pipepipe.InnerTubeLogLevel
import com.metrolist.music.utils.pipepipe.InnerTubeLogger
...
internal class FaradayJsDecoder : YoutubeJavaScriptDecoder {
    ...
    private val engine by lazy { FaradayCipherEngine(HttpClient(CIO), jsThread, bridgedLogger) }
    ...
    override fun decodeBatch(
        ...
        Timber.tag(TAG).d(TAG, "solved $wanted locally (sig=${wantedSignatures.size} n=${wantedNParameters.size})")
    ...
    private val bridgedLogger =
        InnerTubeLogger { event ->
            ...
            when (event.level) {
                InnerTubeLogLevel.DEBUG -> Timber.tag(TAG).d(TAG, message)
                InnerTubeLogLevel.INFO -> Timber.tag(TAG).i(TAG, message)
                InnerTubeLogLevel.WARN -> Timber.tag(TAG).w(TAG, message)
                InnerTubeLogLevel.ERROR -> Timber.tag(TAG).e(TAG, message)
            }
        }
}
