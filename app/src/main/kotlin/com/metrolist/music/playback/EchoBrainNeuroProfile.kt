...
internal class EchoBrainNeuroProfile {
    ...
    /**
     * Performs the once-per-day FlowNeuro maintenance pass.
     *
     * This never invents feedback or downloads data: it only applies a small decay to old
     * signals and compacts the local vector so that the profile remains bounded across restarts.
     */
    @Synchronized
    fun consolidateDaily(): Boolean {
        if (profile.topics.isEmpty()) return false
        val compacted = trimTopics(
            profile.topics.mapValues { (_, value) ->
